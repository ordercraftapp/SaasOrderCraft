...
// ✅ PhaseC: helpers tenant-aware (Web SDK)
...
export default function OrderCardOps({
  ...
  async function handleRemoveItem(index: number) {
    try {
      ...
      // ✅ PhaseC: no escribas si no hay tenant
      if (!tenantId) {
        alert('No se pudo resolver el tenant. Refresca la página e inténtalo de nuevo.');
        return;
      }

      setSaving(true);
      await updateDoc(
        tDoc( 'orders', tenantId, order.id), // ✅ tenants/{tenantId}/orders/{id}
        {
          items: newItems,
          modifiedNote: finalNote,
          updatedAt: serverTimestamp(),
          tenantId, // ✅ persistir tenantId
        }
      );
      // el onSnapshot del padre refresca la UI
    } catch (e: any) {
      console.error(e);
