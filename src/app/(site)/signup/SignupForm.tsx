...
export default function SignupForm({ defaultPlan }: { defaultPlan: PlanId }) {
  ...
  const [headerGapPx, setHeaderGapPx] = useState<number>(112); // valor por defecto razonable
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      // añadimos 24px de espacio extra por si acaso (ajusta si quieres)
      setHeaderGapPx(h + 24);
    };

    update();
    // re-evalúa al redimensionar
  ...
  return (
    <>
      {/* Header fijado arriba — usamos ref para medir su altura */}
      <div ref={headerRef} className="fixed-top" style={{ zIndex: 1030 }}>
        <Header />
      </div>

      {/* Contenedor principal con paddingTop calculado dinámicamente */}
      <div style={{ paddingTop: `${headerGapPx}px` }}>
